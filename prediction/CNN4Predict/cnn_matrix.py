...
def neural_network(window_size=WINDOW_SIZE, kernel_size=KERNEL_SIZE,
...
def make_timeseries_instances(time_series, window_size):
    ...
    x = np.array(
        np.atleast_3d([GAF(time_series[0][start:start + window_size]).gaf()
                       for start in
                       range(0, time_series.shape[1] - window_size)]))
...
def main():
    ...
    rmse = np.sqrt(
        mean_squared_error(predicted_time_series, actual_time_series))
    f = open("evaluate_result/matrix/mse" + str(WINDOW_SIZE) + "_" + str(
        KERNEL_SIZE) + ".txt", "w")
    f.write("Window size " + str(WINDOW_SIZE) + " : " + str(rmse) + "\n")
    f.close()

    # Bieu do
    compared_diagram(predicted_time_series, actual_time_series,
                     str(WINDOW_SIZE) + "_" + str(KERNEL_SIZE))
