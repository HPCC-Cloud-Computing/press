...
class ANFIS:
    ...
    def train(self, x_train, y_train, x_test, y_test, batch_size, epoch, rate):
        ...
        # Cost function
        # cost = tf.reduce_mean(tf.squared_difference(self.output(x, x_test.shape[0]), y))

        # Entire train loss function
        # Phan code nay dung de lay cac gia tri cua train loss, su dung de ve~ do thi hoi tu loss
        loss = tf.reduce_mean(tf.squared_difference(self.output(x, x_train.shape[0]), y))
        # lost_list = []
        ...
        for e in range(epoch):
            ...
            z = net.run(loss, feed_dict={x:x_train, y:y_train})
            print(f'{e}, {z}')
        net.close()
    ...
    def hybrid_sa_training(self, x_train, y_train, x_test, y_test, batch_size, epoch,
        ...
        cost = tf.reduce_mean(tf.squared_difference(self.output(x, batch_size), y))

        # Entire train loss function
        # Phan code nay dung de lay cac gia tri cua train loss, su dung de ve~ do thi hoi tu loss
        # lost = tf.reduce_mean(tf.squared_difference(self.predict(x, x_train.shape[0]), y))
        # lost_list = []
        sa_loss = tf.reduce_mean(tf.squared_difference(self.output(x, x_train.shape[0]), y))

        # Optimizer
        optimizer = tf.train.AdamOptimizer(rate).minimize(cost)
        # Test loss
        # acc la bien dua ra gia tri ve RMSE cua tap test so voi gia tri du doan duoc
        acc = tf.sqrt(tf.reduce_mean(tf.squared_difference(self.output(x, x_test.shape[0]), y)))
        # Su dung pred nay de dua ra duoc MAE vi MAE khong co san trong tensorflow
        # Viec nay khong anh huong nhieu den code performance nen khong can dua het vao tensor cung duoc
    ...
    def predict(self, input_data: np.ndarray):
        ...
        net = tf.InteractiveSession()  
        x = tf.placeholder(dtype=tf.float32, shape=[None, 1, self.window_size])
        predict = self.output(x, input_data.shape[0])
        with tf.name_scope("output_test"):
            net.run(tf.global_variables_initializer())
            result = net.run(predict, feed_dict={x: input_data})
        return result
